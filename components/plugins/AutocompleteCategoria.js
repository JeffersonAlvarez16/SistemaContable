...
class AutoCompleteCategoria extends React.Component {
    state = {
        anchorEl: null,
        dataAuto: [],
        dataAutoTemporal: [],
    ...
    componentWillReceiveProps(props) {
        ...
        if (props.codigocategoria != undefined) {
            firebase.auth().onAuthStateChanged((user) => {
                if (user) {
                    var db = firebase.database();
                    var categoriasRef = db.ref('users/' + user.uid + '/categorias/' + props.codigocategoria);
...
export default AutoCompleteCategoria
